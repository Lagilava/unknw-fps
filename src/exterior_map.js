...
(function () {
  ...
  window.buildExteriorPlayArea = async function (THREE, scene, renderer) {
    ...
    function makeRoadTex(W = 512, H = 512) {
      ...
      grad.addColorStop(0, "rgb(65,69,73)"); grad.addColorStop(1, "rgb(56,60,64)");
    ...
    const M = {
      ...
      ground:    std({ color:0xa5a6aa, roughness:0.96, metalness:0,    map:T.road,     normalMap:T.roadNormal, normalScale:new THREE.Vector2(0.04,0.04), polygonOffset:true, polygonOffsetFactor:1.5, polygonOffsetUnits:1.5 }),
      asphalt:   std({ color:0x888b90, roughness:0.94, metalness:0, map:T.road,     normalMap:T.roadNormal, normalScale:new THREE.Vector2(0.035,0.035) }),
      wetRoad:   std({ color:0x646a70, roughness:0.24, metalness:0, map:T.road,     normalMap:T.wetRoadNormal, normalScale:new THREE.Vector2(0.6,0.6), envMapIntensity:0.7 }),
      ...
      brick:     std({ color:0xd1b3a1, roughness:0.88, metalness:0,    map:T.brick,    normalMap:T.brickNormal, normalScale:new THREE.Vector2(0.45,0.45) }),
      brickDark: std({ color:0xa08e83, roughness:0.90, metalness:0,    map:T.brick,    normalMap:T.brickNormal, normalScale:new THREE.Vector2(0.40,0.40) }),
    ...
    // A texture tile covers a fixed number of metres on every box face.
    // Previously each facade/road slab received the same repeat count regardless
    // of size, stretching brickwork and asphalt across entire city blocks.
    const surfaceTileSize = new Map([
      [M.ground, 6], [M.asphalt, 6], [M.wetRoad, 6], [M.sidewalk, 4],
      [M.concrete, 4], [M.stone, 4], [M.brick, 3], [M.brickDark, 3], [M.wood, 3],
    ]);
    for (const mat of surfaceTileSize.keys()) {
      for (const tex of [mat.map, mat.normalMap]) if (tex) tex.repeat.set(1, 1);
    }
    function mapSurfaceUV(geo, mat) {
      const tile = surfaceTileSize.get(mat);
      if (!tile) return;
    ...
    function setupExtLighting() {
      ...
      scene.add(new THREE.AmbientLight(0xe3eaf0, sunShadows ? 0.18 : 0.24));
      ...
      const hemi = new THREE.HemisphereLight(0xc4d6e6, 0x858278, sunShadows ? 0.65 : 0.75);
    ...
    function buildGround() {
      ...
      // Asphalt stays matte; the individual puddles below supply local reflections.
    ...
    function buildBuilding(cx, cz, w, d, h, bodyMat) {
      ...
      const pickWindow = () => {
        const roll = rng();
        return roll < 0.43 ? M.winLit
          : roll < 0.60 ? M.winLitCool
          : roll < 0.73 ? M.winBlue
          : roll < 0.88 ? M.winDim
          : M.winOff;
      };
      ...
      for (let r = 0; r < rows; r++) {
        for (let c = 0; c < cols; c++) {
          ...
          const wy = 2.0 + r*3.5;
          queueBox(1.30, 1.70, 0.10, wx, wy, faceZ, pickWindow());
        }
      }

      // The old generator dressed only its south face, leaving the skyline as
      // giant beige proxy boxes from most gameplay angles. Dress all visible
      // elevations with a slightly sparser cadence on sides/rears.
      const sideCols = Math.max(2, Math.floor(d / 4.2));
      for (let r = 0; r < rows; r++) {
        const wy = 2.0 + r * 3.5;
        for (let c = 0; c < sideCols; c++) {
          const wz = cz - d * 0.5 + (c + 0.5) * (d / sideCols);
          queueBox(0.10, 1.62, 1.22, cx - w * 0.5 - 0.08, wy, wz, pickWindow());
          queueBox(0.10, 1.62, 1.22, cx + w * 0.5 + 0.08, wy, wz, pickWindow());
        }
        if (r % 2 === 0 || h < 18) {
          for (let c = 0; c < cols; c++) {
            const wx = cx - w * 0.5 + (c + 0.5) * (w / cols);
            queueBox(1.24, 1.58, 0.10, wx, wy, cz + d * 0.5 + 0.08, pickWindow());
          }
        }
      }

      // Corner piers and a shallow crown give each volume a readable frame and
      // keep roof silhouettes from ending as featureless rectangular cutouts.
      for (const sx of [-1, 1]) for (const sz of [-1, 1]) {
        queueBox(0.34, h + 0.5, 0.34, cx + sx * (w * 0.5 - 0.12), (h + 0.5) * 0.5, cz + sz * (d * 0.5 - 0.12), M.metalBase);
      }
      if (w > 13 && h > 18) {
        queueBox(w * 0.62, 1.1, d * 0.62, cx, h + 1.15, cz, bodyMat);
        queueBox(w * 0.66, 0.18, d * 0.66, cx, h + 1.76, cz, M.metalDark);
      }
    ...
    function buildLampposts() {
      const lens = std({ color: 0xffedcc, roughness: 0.38, metalness: 0,
        emissive: 0xffcf88, emissiveIntensity: 0.25 });
      const posts = [
      ...
      for (const [lx, lz] of posts) {
        const poleX = lx + 3.5;
        const towardRoad = poleX < 0 ? 1 : -1;
        const groundY = poleX < 0 ? 0.095 : 0;
        queueCyl(0.06, 0.08, 7 - groundY, poleX, (7 + groundY) / 2, lz, M.lampPole, 8);
        // A horizontal arm joins the pole to the housing. The old second
        // cylinder stood vertically beside the pole, leaving both parts floating.
        queueBox(1.3, 0.10, 0.10, poleX + towardRoad * 0.65, 6.95, lz, M.lampPole);
        queueBox(0.72, 0.22, 0.40, poleX + towardRoad * 1.3, 6.99, lz, M.metalDark);
        queueBox(0.58, 0.025, 0.29, poleX + towardRoad * 1.3, 6.885, lz, lens);
        addCollider(poleX, lz, 0.2, 0.2);
      }
    }
    ...
    function buildStreetClutter() {
      ...
      makePowerLine(-43, 13.5, 149, -14, 7.0, 145, 0.8);
      makePowerLine(-43, 13.5, 149, -14, 7.0, 160, 1.2);
      makePowerLine( 43, 13.5, 149,  21, 7.0, 145, 0.8);
      makePowerLine( 43, 13.5, 149,  21, 7.0, 160, 1.2);
      makePowerLine(-45, 20.0, 192, -14, 7.0, 192, 1.5);
      makePowerLine( 45, 14.0, 192,  21, 7.0, 192, 1.3);
