...
import { RELAY_UPLOAD_SECONDS } from './gameplay_pacing';

export interface GameHudState {
  hp: number; maxHp: number; unlimitedHealth: boolean;
  killed: number; total: number; grenadeCooldown: number;
  relayProgress?: number; contested?: boolean; blackout?: boolean; objectiveDistance?: number; spawnShieldSeconds?: number; pvp: boolean;
}
...
export function createGameHud(doc = document) {
  ...
  const wallet = make('wallet-readout', 'XP AVAILABLE');
  ...
  style.textContent = `
    ...
    #hud.combat-hud #minimap-canvas { top:42px; right:max(28px,env(safe-area-inset-right)); width:210px; height:210px; transform:none!important; box-shadow:0 0 0 1px var(--ui-line),0 12px 34px rgba(0,0,0,.38); }
    #tracker-caption { position:absolute; right:28px; top:23px; width:210px; text-align:center; color:#ddd6c9; letter-spacing:.12em; font:700 9px/1.4 'Segoe UI',sans-serif; text-shadow:0 1px 3px #000; border-bottom:1px solid #aaa39755; padding-bottom:5px; }
    ...
    #wallet-readout { grid-column:1 / -1; display:flex; align-items:center; justify-content:space-between; gap:12px; border-top:1px solid var(--ui-line); padding-top:7px; margin-top:3px; color:var(--site-yellow); font:700 10px/1.4 'Segoe UI',sans-serif; letter-spacing:.08em; }
    #hud.combat-hud #xp-val { color:#fff; font:700 22px/1 'Segoe UI',sans-serif; font-variant-numeric:tabular-nums; letter-spacing:0; }
    ...
    /* A blackout is played without usable instrumentation. The sparse, unstable
       interference flashes confirm that the HUD failed instead of looking like a
       CSS bug, but never reveal health, ammo, radar, objective, or a crosshair. */
    body.blackout-hud-disrupted #hud.combat-hud > * { visibility:hidden!important; }
    body.blackout-hud-disrupted #xhair,
    body.blackout-hud-disrupted #scope-overlay,
    body.blackout-hud-disrupted #touch-controls { visibility:hidden!important; }
    body.blackout-hud-disrupted #hud.combat-hud::before,
    body.blackout-hud-disrupted #hud.combat-hud::after {
      display:block!important; content:''; position:fixed; inset:0; pointer-events:none;
      visibility:visible!important; z-index:9997; opacity:.2; mix-blend-mode:screen;
    }
    body.blackout-hud-disrupted #hud.combat-hud::before {
      background:repeating-linear-gradient(0deg,transparent 0 13px,rgba(130,225,255,.16) 14px,transparent 16px),
        linear-gradient(90deg,transparent 0 18%,rgba(180,240,255,.16) 18.3% 18.8%,transparent 19.2% 72%,rgba(255,255,255,.1) 72.2% 73%,transparent 73.3%);
      clip-path:polygon(0 8%,100% 8%,100% 11%,0 11%,0 39%,100% 39%,100% 42%,0 42%,0 78%,100% 78%,100% 80%,0 80%);
      animation:blackoutHudTear .19s steps(2,end) infinite;
    }
    body.blackout-hud-disrupted #hud.combat-hud::after {
      inset:auto 7vw 8vh auto; width:150px; height:2px; background:#b7edff;
      box-shadow:0 -42vh 0 rgba(183,237,255,.35),-72vw -18vh 0 rgba(183,237,255,.25);
      animation:blackoutHudDrop 1.7s steps(1,end) infinite;
    }
    @keyframes blackoutHudTear { 0%{transform:translate(0)} 35%{transform:translate(7px,-2px)} 70%{transform:translate(-11px,3px)} 100%{transform:translate(2px)} }
    @keyframes blackoutHudDrop { 0%,82%{opacity:0} 84%{opacity:.48} 87%{opacity:.08} 91%{opacity:.32} 94%,100%{opacity:0} }
    @media(max-width:600px) {
      #hud.combat-hud #center-hud { left:12px; top:12px; width:calc(100% - 176px); padding:10px; }
      ...
      #hud.combat-hud #minimap-canvas { right:12px; top:32px; width:142px; height:142px; transform:none!important; }
      #tracker-caption { right:12px; top:14px; width:142px; font-size:8px; }
    ...
    @media(max-height:500px) {
      ...
      #hud.combat-hud #minimap-canvas { width:120px; height:120px; top:28px; }
      #tracker-caption { top:10px; width:120px; }
  ...
  return (state: GameHudState) => {
    ...
    const conditionName = state.unlimitedHealth || (state.spawnShieldSeconds || 0) > 0 ? 'protected' : state.hp <= 0 ? 'down' : ratio <= .3 ? 'critical' : ratio <= .6 ? 'wounded' : 'stable';
    if (root.dataset.health !== conditionName) root.dataset.health = conditionName;
    setTextIfChanged(condition, conditionName === 'protected' && (state.spawnShieldSeconds || 0) > 0
      ? `Wave shield ${Math.ceil(state.spawnShieldSeconds!)}s`
      : { protected:'Invulnerable', down:'Downed', critical:'Find cover', wounded:'Wounded', stable:'Stable' }[conditionName]);
    ...
    setTextIfChanged(tracker, relay && Number.isFinite(state.objectiveDistance)
      ? `OBJECTIVE  ${Math.round(state.objectiveDistance!)}m`
      : 'TACTICAL MAP');
    ...
    setTextIfChanged(remaining, relay ? `${Math.floor(state.relayProgress!)} / ${RELAY_UPLOAD_SECONDS}s secured` : `${Math.max(0, state.total - state.killed)} HOSTILES`);
    const value = Math.round(Math.max(0, Math.min(1, relay ? state.relayProgress! / RELAY_UPLOAD_SECONDS : state.total > 0 ? state.killed / state.total : 0)) * 100);
