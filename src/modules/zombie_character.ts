...
export function createZombieCharacter(THREE: any, sourceScene: any, clips: any, skeletonClone: any, options: any = {}) {
  ...
  let prevHitFlash = 0;
  let hitFlinch = 0;
  // Restore mixer poses before composing offsets, even on unkeyed bones.
  const offsetBones = [...new Set([spine, spine2, neck, head, hips, lShldr, rShldr, lFore, rFore].filter(Boolean))];
  const mixerPose = offsetBones.map(b => b.quaternion.clone());
  ...
  // Alive one-shots cap the lock at 1.1s: the intimidation scream must be a snappy
  ...
  function playOnce(name, fade = 0.16) {
    ...
    const lock = (name === "death" || name === "dying") ? dur - fade : Math.min(dur, 1.1);
    actions[name].timeScale = name === "death" || name === "dying" ? 1 : dur / Math.max(0.1, lock);
  ...
  function setDead(v = true) {
    ...
    if (dead) return;
    dead = true;
    if (upperAction) upperAction.stop();
    upperAction = null;
  ...
  function update(dt, enemyRef) {
    offsetBones.forEach((b, i) => b.quaternion.copy(mixerPose[i]));
    if (dt > 0.1) dt = 0.1;
    elapsed += dt;
    ...
    if (dead) { mixer.update(dt); offsetBones.forEach((b, i) => mixerPose[i].copy(b.quaternion)); model.position.y = baseModelY; return; }

    const hitFlash = enemyRef?.hitFlash || 0;
    if (hitFlash > prevHitFlash + 0.05) hitFlinch = Math.min(0.22, hitFlinch + 0.16);
    prevHitFlash = hitFlash;
    hitFlinch = damp(hitFlinch, 0, 12, dt);
    ...
    if (isAttacking && !prevAttacking) {
      ...
      const pick = attackUpperNames.includes(enemyRef?.attackVariant) ? enemyRef.attackVariant : attackUpperNames.length
    ...
    if (elapsed >= onceLockUntil) {
      ...
      else if (visualSpeed < 0.08) want = "idle";
    ...
    if (activeAction && LOCO_STATES.has(activeName)) {
      ...
      activeAction.timeScale = activeName === "idle" ? speedJitter
        : clamp(visualSpeed / Math.max(0.001, baseSpeed), 0, 3.5);
    }

    mixer.update(dt);
    offsetBones.forEach((b, i) => mixerPose[i].copy(b.quaternion));
    ...
    addBoneOffset(spine2, hunch * 0.5 - hitFlinch, sway, leanTurn * 0.6 + limpSide * limp * 0.06);
  ...
  function dispose() {
    ...
    // Clones share geometry/materials with the cache; only skeleton buffers belong
    // to this instance. Disposing shared assets causes reuploads for the horde.
    const skeletons = new Set<any>();
