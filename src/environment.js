...
(function () {
  ...
  function setupEnvironmentLighting(THREE, scene, renderer) {
    ...
    // 0.55 folds in the old separate `moonlight` fill (buildEveningSky, removed —
    // a second near-identical warm overhead DirectionalLight covering the same
    // whole-scene area with no falloff) so the combined wash intensity is unchanged.
    const ceilFill = tagEnvironmentLight(
      new THREE.DirectionalLight(0xfff3e0, profile.dir * (OUTDOOR_MODE ? 0.55 : 1.35))
    );
  ...
  // Formerly added a second whole-scene warm DirectionalLight fill here (`moonlight`);
  // folded into setupEnvironmentLighting's `ceilFill` instead (same uniform, no-falloff
  // coverage, so one light does the job — see the comment at ceilFill's definition).
  // ---------------------------------------------------------------------------
  ...
  function buildArenaFinishPass(THREE, scene, wallPositions) {
    ...
    function instanceBoxes(geo, mat, positions, y) {
      ...
      for (let i = 0; i < positions.length; i++) {
        matrix.makeTranslation(positions[i].x, y, positions[i].z);
        mesh.setMatrixAt(i, matrix);
      }
      mesh.instanceMatrix.needsUpdate = true;
    ...
    // lines across the entire arena.
    const routeSegments = [];
    for (let z = -28; z <= 108; z += 13.5) routeSegments.push({ x: 0, z, w: 0.18, d: 6.2 });
    for (const x of [-44, 0, 44]) routeSegments.push({ x, z: 116.9, w: 7.2, d: 0.22 });
    for (const seg of routeSegments) {
      const mark = new THREE.Mesh(new THREE.PlaneGeometry(seg.w, seg.d), routeMat);
      mark.rotation.x = -Math.PI * 0.5;
      mark.position.set(seg.x, 0.018, seg.z);
      mark.receiveShadow = true;
      scene.add(mark);
    }

    // Hazard ticks visually explain the three exterior breach points and make the
    // arena-to-street transition legible from across the map.
    for (const exitX of [-44, 0, 44]) {
      for (let i = -3; i <= 3; i++) {
        const tick = new THREE.Mesh(new THREE.BoxGeometry(0.48, 0.035, 1.5), i % 2 ? darkMetal : cautionMat);
        tick.position.set(exitX + i * 0.72, 0.035, 117.1);
        tick.rotation.y = -0.38;
        tick.receiveShadow = true;
        scene.add(tick);
      }
    }

    function makeSectorSign(text, x, z, ry) {
      const canvas = document.createElement("canvas");
    ...
    for (const [x, z] of [[-65.95,8],[-65.95,62],[65.95,35],[65.95,94]]) {
      const box = new THREE.Mesh(new THREE.BoxGeometry(0.11, 1.35, 1.05), darkMetal);
      box.position.set(x, 1.15, z); box.castShadow = true; scene.add(box);
      const strip = new THREE.Mesh(new THREE.BoxGeometry(0.125, 0.08, 0.74), cautionMat);
      strip.position.set(x + (x < 0 ? 0.015 : -0.015), 1.48, z); scene.add(strip);
    }
  }
