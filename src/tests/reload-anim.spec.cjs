...
test("reload stages the gun in the left hand + a fresh clip in the right hand", async ({ page }) => {
  test.setTimeout(300000); // includes cold FBX boot and software-rendered sampling
  ...
  // Advance the real reload controller at a fixed cadence so a fast reload
  // cannot skip the early magazine phase between slow software-rendered frames.
  await page.evaluate(() => {
    window.__rbTest.pauseForAnimationReview(true);
    window.__rbTest.startReloadForTest();
  });
  ...
  for (let i = 0; i < 400; i++) {
    const r = await page.evaluate(() => window.__rbTest.tickReloadForTest(1 / 120));
  ...
  await page.evaluate(() => window.__rbTest.tickReloadForTest(1 / 120));
