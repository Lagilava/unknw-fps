...
test('ability FX render, stay bounded, and expire',async({page})=>{
 ...
 const report=await page.evaluate(async()=>{
  const THREE=await import('three');const {createBoltCore,createAbilityImpacts}=await import('/src/modules/ability_fx');
  ...
  scene.add(new THREE.HemisphereLight(0xb8dfff,0x182030,2)); const key=new THREE.DirectionalLight(0xffffff,3);key.position.set(-3,8,5);scene.add(key);
 ...
 expect(report).toEqual({vertices:144,finite:true,active:3});
